...
export default class SelectableSectionsListView extends Component {
  ...
  renderRow(item, sectionId, index) {
    ...
    const isLast =
      this.sectionItemCount && this.sectionItemCount[sectionId] - 1 === index
  ...
  render() {
    ...
    if (this.props.initialListSize) {
      props = merge(props, { initialListSize: this.props.initialListSize })
    }
...
SelectableSectionsListView.propTypes = {
   ...
   * items. If not provided, the section ids will be used 
   * (the keys from the data object)
   */
  getSectionTitle: PropTypes.func,
  getSectionListTitle: PropTypes.func,

  /**
   * Function to sort sections. If not provided, 
   * the sections order will match data source
   */
